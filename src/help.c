...
static const char *Help_GetRTF(HelpTopic topic)
{
    ...
    case HELP_INTERFACE:
        return "{\\rtf1\\ansi\\deff0"
            ...
            "{\\b Script Button Tinting}\\par\\par"
    ...
    case HELP_RUNNING_SCRIPTS:
        return "{\\rtf1\\ansi\\deff0"
            ...
            "\\bullet  Press {\\b Escape} to cancel repeat and stop the currently running background script.\\par"
            ...
            "the newest version of a script before execution.\\par\\par"
            "{\\b Script Output Log}\\par\\par"
            "When scripts run in background mode (no console), all stdout and stderr output is "
            "captured and shown in the Script Output Log window.\\par\\par"
            "\\bullet  Click {\\b \\u8801?  Log} in the toolbar to open the log window at any time.\\par"
            "\\bullet  Each run is framed by a header and a completion footer:\\par"
            "{\\f1\\cf3   === ScriptName  (HH:MM:SS) ===}\\par"
            "{\\f1\\cf3   --- Finished successfully. ---}\\par\\par"
            "The footer reads {\\f1\\cf3 --- Stopped by user. ---} if the script was terminated "
            "with the {\\b Stop} button, or {\\f1\\cf3 --- Exited with code N. ---} for a non-zero exit.\\par\\par"
            "{\\b Syntax highlighting}\\par\\par"
            "\\bullet  {\\b Header lines} (===) \\emdash accent blue\\par"
            "\\bullet  {\\b Finished successfully} \\emdash green\\par"
            "\\bullet  {\\b Stopped by user} \\emdash amber\\par"
            "\\bullet  {\\b Exited with code N} \\emdash red\\par"
            "\\bullet  Lines containing Error, Exception, Traceback, Fatal \\emdash red\\par"
            "\\bullet  Lines containing Warning \\emdash amber\\par\\par"
            "\\bullet  Output is buffered internally \\emdash nothing is lost if the window is closed during a run.\\par"
            "\\bullet  The window follows the current app theme (dark/light) and uses a Consolas monospace font.\\par\\par"
            "> Note: Output is only captured in background mode. When {\\b Show console} is enabled "
            "in Settings, output appears in the console window instead.\\par"
            "}";
    ...
    case HELP_SETTINGS:
        return "{\\rtf1\\ansi\\deff0"
            ...
            "Both buttons open a dialog listing each extra repository and local folder with its own checkbox, "
            "so you choose exactly which items to include. A {\\b General settings} checkbox and an "
            "{\\b Include tokens} toggle are also available.\\par\\par"
            "{\\b Export:} uncheck any repos or folders you do not want. Uncheck {\\b Include tokens} "
            "to strip all tokens so the file is safe to share. Then choose a destination {\\f1\\cf3 .ini} file.\\par\\par"
            "{\\b Import:} choose the source file first, then the dialog shows the repos and folders "
            "found in that file. Selected items are {\\b appended} to your current sources \\emdash nothing "
            "is removed. Duplicate URLs are skipped silently; if a duplicate carries a token you are prompted "
