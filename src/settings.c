...
#define WB(sec, key, val) WritePrivateProfileString(sec, key, (val) ? L"1" : L"0", ini) /* writes "1" or "0" for each boolean flag */
    ...
    for (int i = 0; i < s->extra_repo_count; i++)
    {
        ...
        _snwprintf_s(key, 31, _TRUNCATE, L"Repo%dToken", i);
        WritePrivateProfileString(L"Sources", key, s->extra_repos[i].token, ini);
        _snwprintf_s(key, 31, _TRUNCATE, L"Repo%dEnabled", i);
        WritePrivateProfileString(L"Sources", key, s->extra_repos[i].enabled ? L"1" : L"0", ini);
    }
    ...
    for (int i = 0; i < s->local_dir_count; i++)
    {
        ...
        _snwprintf_s(key, 31, _TRUNCATE, L"Local%dEnabled", i);
        WritePrivateProfileString(L"Sources", key, s->local_dirs[i].enabled ? L"1" : L"0", ini);
    }
                                                ...
                                                WPARAM wp, LPARAM lp)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK:
        {
            ...
            bool any = p->incl_python_path || p->incl_cache_dir || p->incl_options || p->main_token_selected;
...
INT_PTR CALLBACK SettingsDlgProc(HWND hwnd, UINT msg, WPARAM wp, LPARAM lp)
{
    ...
    case WM_NOTIFY:
    {
        ...
        { /* TCN_SELCHANGE fires when the user clicks a tab */
    ...
    case WM_COMMAND:
        ...
        case IDC_BTN_BROWSE_PY:
        {
            ...
            OPENFILENAME ofn = {
                .lStructSize = sizeof(ofn), .hwndOwner = hwnd, .lpstrFilter = L"Python\0python.exe;python3.exe\0All Files\0*.*\0", .lpstrFile = path, .nMaxFile = MAX_APPPATH, .Flags = OFN_FILEMUSTEXIST | OFN_PATHMUSTEXIST, .lpstrTitle = L"Select Python Executable"};
        ...
        case IDOK:
        {
            ...
            if (IsDlgButtonChecked(hwnd, IDC_RAD_THEME_DARK) == BST_CHECKED)
                s->theme = THEME_DARK;
            else if (IsDlgButtonChecked(hwnd, IDC_RAD_THEME_LIGHT) == BST_CHECKED)
                s->theme = THEME_LIGHT;
            else
                s->theme = THEME_SYSTEM; /* checked radio wins; fall through to THEME_SYSTEM */
            if (IsDlgButtonChecked(hwnd, IDC_RAD_SORT_ALPHA) == BST_CHECKED)
                s->sort_mode = SORT_ALPHA;
            else if (IsDlgButtonChecked(hwnd, IDC_RAD_SORT_DATE) == BST_CHECKED)
                s->sort_mode = SORT_DATE;
            else if (IsDlgButtonChecked(hwnd, IDC_RAD_SORT_USED) == BST_CHECKED)
                s->sort_mode = SORT_MOST_USED;
            else
    ...
    case WM_COMMAND:
        ...
        case IDCANCEL:
            if (s_hFontTitle)
            {
                ...
                s_hFontTitle = NULL;
            } /* free GDI font to avoid leak */
            if (s_hIconApp)
            {
                DestroyIcon(s_hIconApp);
                s_hIconApp = NULL;
            } /* free icon handle to avoid leak */
            EndDialog(hwnd, IDOK);
            break;
        }
        return TRUE;
